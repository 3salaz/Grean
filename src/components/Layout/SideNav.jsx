...
const SideNav = ({ isOpen, toggleMobileNav }) => {
  ...
  return (
    <motion.div
      ...
      variants={variants}
      className="fixed top-[8svh] left-0 h-[92%] w-[65%] bg-gray-800 rounded-r-lg drop-shadow-2xl z-20 pb-4"
    >
      <div className="flex flex-col justify-between p-4 w-full h-full bg-white bg-opacity-95 rounded-br-md">
        <div className="text-center flex gap-2 items-center justify-start h-[10%] flex-wrap">
          <Link to="/" className="h-full w-auto flex">
            <img className="h-full rounded-full" src={logo} alt="logo" />
          </Link>
          <div className="text-grean font-bold text-3xl">Grean</div>
        </div>

        <ul className="space-y-5 pt-16">
          <li>
            <Link
